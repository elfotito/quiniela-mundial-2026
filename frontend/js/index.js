...
const LV_ESTADIOS = [
  { img:"https://digitalhub.fifa.com/transform/cd18bc06-f27b-49ff-8ec7-a6e9c195e200/England-v-Congo-DR-Round-Of-32-FIFA-World-Cup-2026?focuspoint=0.5,0.0&io=transform:fill,aspectratio:1x1,width:1536&quality=75", badge:"Remontada Epica", fact:"Kane salva a Inglaterra, una vez más. Los analistas ya lo dan como fuerte candidato a Balon de Oro" },
...
async function iniciarCountdown() {
    ...
    const partidoTitulo = document.querySelector('.countdown-label p');
    ...
    async function obtenerProximoPartido() {
        try {
            ...
            if (!proximoPartido || proximoPartido.id !== data.partido.id) {
                ...
                console.log('Fecha:', data.partido.fecha);
    ...
    function actualizarCountdown() {
        ...
        // Si llegó a 0 o menos, buscar siguiente partido
        if (distancia <= 0) {
    ...
    if (proximoPartido) {
        ...
        // Verificar cada 30 segundos si hay un partido más cercano
        setInterval(async () => {
            await obtenerProximoPartido();
        }, 30000);
    }
...
function renderNoticia(n) {
    ...
    if (n.tipo === 'hero') {
        const img = n.imagen_url
            ? `<img style="width:100%;height:300px;object-fit:cover;object-position:top;display:block;" src="${n.imagen_url}" alt="${n.titulo}" onerror="this.style.display='none'">`
...
function obtenerBandera(nombre) {
    const banderas = {
        ...
        'Republica Checa': '🇨🇿', 'Irlanda del Norte': '🏴󠁧󠁢󠁮󠁩󠁲󠁿',
        // Asia
